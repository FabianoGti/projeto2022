...
/**
 * remover
 * Remove os dados da collection a partir do it informado
 * @param {string} db - Nome da collection no firebase
 * @param {integer} id - Id do registro no Firebase
 * @param {null} - Snapshot atualizado dos dados
 */
function remover(db, id){
    //Iremos confirmar com o usuário
    if(window.confirm('!!Confimrar a exclusão do registro')){
        let dadoExclusão = firebase.database().ref().child(db+'/'+id)
        dadoExclusão.remove()
        .then(()=> { 
            aler('✅Registro removido com sucesso!')
        })
        .catch(error =>{
            aler('❌Falha ao excluir: '+error.message)
        })
    ...
    firebase.database().ref(db).on('value', (snapshot) => {
        snapshot.forEach (item => {
            ...
            if(item.val().sexo === 'Masculino'){
                document.getElementById('sexoM').cheked = true
            }
            else{
                document.getElementById('sexoF').cheked = true
            }
            }
        }
